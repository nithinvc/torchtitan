...
class ConvCNPWeatherE2E(nn.Module):
    ...
    def __init__(
        ...
        self.forecast_model = nn.ModuleList(
            [self.load_forecast_model(forecast_model_path, l + 1) for l in range(lead_time)]
        )
        ...
        self.forecast_input_means = (
            self.to_tensor(np.load(aux_data_path + "norm_factors/mean_4u_1.npy")).unsqueeze(0).unsqueeze(0).unsqueeze(0)
        )
        self.forecast_input_stds = (
            self.to_tensor(np.load(aux_data_path + "norm_factors/std_4u_1.npy")).unsqueeze(0).unsqueeze(0).unsqueeze(0)
        )
    ...
    def load_sf_model(self, sf_model_path, lead_time):
        ...
        best_epoch = np.argmin(np.load("{}/lt_{}/losses_0.npy".format(sf_model_path, lead_time)))
    ...
    def process_forecast_output(self, task, x, last=False):
        ...
        base_context = (base_context * self.forecast_input_stds + self.forecast_input_means).permute(0, 3, 2, 1)
    ...
    def forward(self, task, film_index=None):
        ...
        if self.return_gridded:
            initial_state = initial_state.permute(0, 3, 2, 1) * self.forecast_input_stds + self.forecast_input_means
